...
impl Client {
	...
	pub fn service_fee(&self, request: &mut ServiceFeeRequest) -> (ServiceFeeResponse, Option<Box<dyn Error>>) {
		let mut response = ServiceFeeResponse::default();
		let response_err: Result<(), Box<dyn Error>>;
		...
		if !request.terminal_name.is_empty() {
            match self.resolve_terminal_route(&request.terminal_name) {
                Ok(route) => {
                    if route.cloud_relay_enabled {
                        response_err = self.relay_request("/api/service-fee", "POST", &request, &mut response, request.test, Some(request.timeout));
                    } else {
                        let auth_request = TerminalServiceFeeRequest {
                            api_credentials: route.transient_credentials.clone().unwrap_or_default(),
                            request: request.clone(),
                        };
                        response_err = self.terminal_request(route, "/api/service-fee", "POST", &auth_request, &mut response, Some(request.timeout));
                    }
                }
                Err(e) => {
                    if e.downcast_ref::<ErrUnknownTerminal>().is_some() {
			...
			response_err = self.gateway_request("/api/service-fee", "POST", request, &mut response, request.test, Some(request.timeout));
		}
