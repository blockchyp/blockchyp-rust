...
fn delete_merchant_platforms_example() -> Result<(), Box<dyn Error>> {
    // sample credentials
    ...
    let request = blockchyp::MerchantPlatformRequest{
        platform_id: "XXXXXXXXXXXXX".to_string(),
        ..Default::default()
    };
    let (response, err) = client.delete_merchant_platforms(&request);
...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    delete_merchant_platforms_example()?;
