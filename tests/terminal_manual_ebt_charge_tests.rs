...
#[test]
fn test_terminal_manual_ebt_charge() {
    let config = test_utils::load_test_configuration();
    let client = config.new_test_client(Some(""));

    test_utils::process_test_delay(&config, "TerminalManualEBTCharge");
    ...
    assert_eq!(response.authorized_amount, "27.00");
    assert_eq!(response.remaining_balance, "73.00");
}
